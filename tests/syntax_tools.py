...
class TestSyntaxTools(unittest.TestCase):
    ...
    def test_cse_let_handling(self):
        ...
        e = syntax.ELet(e,
            syntax.ELambda(y,
                syntax.ELet(e,
                    syntax.ELambda(z, syntax.EBinOp(z, "+", y)))))

        assert retypecheck(e)

        print(pprint(e))
        e_prime = cse(e)
        print(pprint(e_prime))

        expected = ELet(EBinOp(x, "+", ONE).with_type(INT), ELambda(y, EBinOp(y, "+", y).with_type(INT))).with_type(INT)

        assert alpha_equivalent(e_prime, expected)
